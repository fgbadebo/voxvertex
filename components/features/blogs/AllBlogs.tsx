...
import { allBlogsList, blogsCategoryList } from "@/constants";
...
export default function AllBlogs() {
  ...
  const filteredBlogs =
    selectedCategory === "View all"
      ? allBlogsList
      : allBlogsList.filter((blog) => blog.category === selectedCategory);

  const visibleBlogs = showAll ? filteredBlogs : filteredBlogs.slice(0, 8);
  ...
  return (
    <section ref={sectionRef} className="relative flex flex-col items-center justify-center py-7 gap-12 lg:gap-8 xl:gap-12">
      <div className="w-full flex flex-col gap-6 md:gap-8 lg:gap-6 xl:gap-8">
        <div className="w-full flex flex-col gap-3 md:gap-4 lg:gap-3 xl:gap-4">
          ...
          <div className="flex flex-wrap gap-3 md:gap-4 lg:gap-3 xl:gap-4">
            {blogsCategoryList.map((category) => (
              <button
                key={category}
                onClick={() => {
                ...
                className={cn(
                  ...
                  selectedCategory === category
                    ...
                    : "bg-transparent text-primary hover:text-background hover:bg-primary" // Inactive Styles
                )}
              >
                {category}
              </button>
