...
import { allPodcastsList, podcastsCategoryList } from "@/constants";
...
const PodcastCard = ({ data }: { data: podcastData }) => {
      ...
      <article className="flex flex-col transition-all duration-300 h-full">
        ...
        <div className="flex flex-col grow bg-card p-4 lg:p-2 xl:p-3 pb-6 lg:pb-4 xl:pb-6 text-secondary transition-all duration-300 gap-8 lg:gap-6 xl:gap-8 rounded-b-2xl md:rounded-b-xl lg:rounded-b-xl xl:rounded-b-2xl justify-between">
          ...
          <div className="flex items-center justify-between mt-auto">
            ...
            <div className="flex gap-4 md:gap-6 lg:gap-4 xl:gap-6">
              <button
              ...
              >
                <Heart
                  animateOnTap
                  className={cn(
                    "stroke-[1px] transition-colors duration-300",
                    isLiked
                      ? "fill-destructive text-destructive"
                      : "fill-none text-secondary group-hover:text-destructive"
                  )}
                />
              </button>
            </div>
...
export default function AllPodcasts() {
  ...
  const filteredPodcasts =
    selectedCategory === "View all"
      ? allPodcastsList
      : allPodcastsList.filter(
          (podcast) => podcast.category === selectedCategory
        );

  const visiblePodcasts = showAll
    ? filteredPodcasts
    : filteredPodcasts.slice(0, 8);
    ...
    >
      <div className="w-full flex flex-col gap-6 md:gap-8 lg:gap-6 xl:gap-8">
        <div className="w-full flex flex-col gap-3 md:gap-4 lg:gap-3 xl:gap-4">
          ...
          <div className="flex flex-wrap gap-3 md:gap-4 lg:gap-3 xl:gap-4">
            {podcastsCategoryList.map((category) => (
              <button
                key={category}
                onClick={() => {
      ...
      {filteredPodcasts.length > 8 && (
        ...
        >
          ...
          >
            {showAll ? "Show Less" : "View all podcasts"}
          </button>
        </AnimatedContent>
      )}
    </section>
